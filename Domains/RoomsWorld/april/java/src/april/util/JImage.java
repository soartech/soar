...
/** JComponent used to view an image. **/
public class JImage extends JComponent
{
    ...
    public synchronized void paint(Graphics gin)
    {
        ...
        if (flipy) {
            AffineTransform gt = (AffineTransform) t.clone();
            gt.concatenate(AffineTransform.getTranslateInstance(0, im.getHeight()));
            gt.concatenate(AffineTransform.getScaleInstance(1, -1));
            g.drawImage(im, gt, null);
        } else {
    ...
    */
    ...
    public void saveImage()
    {
        ...
        try {
            ...
            System.out.println("Wrote image: "+path);
        } catch (IOException ex) {
            System.out.println("Error writing image: "+ex);
        }
    }
